...
public class CommandManager {
    ...
    public void handleCommand(String string) {
        ...
        if (params.get(0).isEmpty()) {
            ChatUtil.sendFormatted(String.format("%sUnknown command&r", OpenMyau.clientName).replace("&", "§"));
        } else {
            for (Command command : OpenMyau.commandManager.commands) {
            ...
            ChatUtil.sendFormatted(String.format("%sUnknown command (&o%s&r)&r", OpenMyau.clientName, params.get(0)).replace("&", "§"));
        }
    }
