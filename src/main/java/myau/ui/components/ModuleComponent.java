...
public class ModuleComponent implements Component {
    ...
    public ModuleComponent(Module mod, CategoryComponent category, int offsetY) {
        ...
        int y = offsetY + 12;
        if (!OpenMyau.propertyManager.properties.get(mod.getClass()).isEmpty()) {
            for (Property<?> baseProperty : OpenMyau.propertyManager.properties.get(mod.getClass())) {
    ...
    public void draw(AtomicInteger offset) {
        ...
        if (this.mod.isEnabled()) {
            textColor = ((HUD) OpenMyau.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis(), offset.get()).getRGB();
        } else {
