...
/**
 * BED ESP — skidded from Raven B4 (keystrokesmod) into OpenMyau.
 * Bed bounding-box rendering (Static/Gradient/Rainbow) plus a bed defense-layer
 * analysis overlay (block counts / best break tools) fed by SharedBlockHighlightCache.
 */
public class BedESP extends Module {
    private static final Minecraft mc = Minecraft.getMinecraft();
    /** Compatibility field: OpenMyau MixinBlockRendererDispatcher writes bed heads here. */
    ...
    public final ColorProperty color2 = new ColorProperty("Color 2", 0x5555FF, () -> this.colorMode.getValue() == 1);
    public final FloatProperty gradientSpeed = new FloatProperty("Gradient speed", 1.0F, 0.1F, 8.0F, () -> this.colorMode.getValue() == 1);
    ...
    public final ColorProperty exposedOutlineColor = new ColorProperty("Exposed outline color", 0xFFDC40, () -> this.showExposedOutline.getValue());
    public final BooleanProperty showDefenseLayers = new BooleanProperty("Show defense layers", true);
    public final BooleanProperty showDefenseTools = new BooleanProperty("Show break tools", false, () -> this.showDefenseLayers.getValue());
    public final BooleanProperty showDefenseCounts = new BooleanProperty("Show defense counts", true, () -> this.showDefenseLayers.getValue() && !this.showDefenseTools.getValue());
    public final FloatProperty defenseHeight = new FloatProperty("Defense height", 0.6F, 0.1F, 3.0F, () -> this.showDefenseLayers.getValue());
    public final FloatProperty defenseScale = new FloatProperty("Defense scale", 1.0F, 0.1F, 2.0F, () -> this.showDefenseLayers.getValue());
    public final BooleanProperty defenseAutoScale = new BooleanProperty("Auto Scale", false, () -> this.showDefenseLayers.getValue());
    ...
    public int getScanSpeedBudget() {
        // Raven B4: budget = scanSpeed * 5000 (section budget per client tick)
        return isEnabled() ? (int) (scanSpeed.getValue() * 5000.0F) : 0;
    }
    ...
    public void onRender3D(Render3DEvent e) {
        ...
        for (BlockPos[] pair : activeBedPairs) {
            ...
            if (!RenderUtil.isInViewFrustum(bb, 0)) {
                continue;
            }
    ...
    private void renderDefenseEntry(DefenseOverlayEntry entry, int iconX, int iconY) {
        ...
        if (entry.hasItemStack()) {
            RenderUtil.renderItemInGUI(entry.renderStack, iconX, iconY);
            return;
        }
    ...
    private void renderDefenseBackground(int left, int top, int right, int bottom) {
        drawRoundedGradientOutlinedRectangle(left, top, right, bottom, DEFENSE_BACKGROUND_RADIUS,
    ...
    /** Compatibility method used by BedNuker for target-box height. */
    ...
    public int getCurrentColor() {
        ...
        switch (mode) {
            case 0:
                return mergeAlpha(color.getValue(), 64);
            case 1: {
                double pct = Math.sin(System.currentTimeMillis() * gradientSpeed.getValue() / 1000.0) * 0.5 + 0.5;
                Color c1 = new Color(color.getValue());
                Color c2 = new Color(color2.getValue());
                Color blended = convert(c1, c2, pct);
                int a = (int) (64 * pct + 64 * (1 - pct));
                return mergeAlpha(blended.getRGB(), a);
            }
            case 2:
                return mergeAlpha(getChroma(2L, 0L), 64);
            default:
                return mergeAlpha(color.getValue(), 64);
        }
    }

    private static int mergeAlpha(int rgb, int alpha) {
        return (rgb & 0xFFFFFF) | (alpha << 24);
    }

    private static Color convert(Color c1, Color c2, double pct) {
        int r = (int) (c1.getRed() + (c2.getRed() - c1.getRed()) * pct);
        int g = (int) (c1.getGreen() + (c2.getGreen() - c1.getGreen()) * pct);
        int b = (int) (c1.getBlue() + (c2.getBlue() - c1.getBlue()) * pct);
        int a = (int) (c1.getAlpha() + (c2.getAlpha() - c1.getAlpha()) * pct);
        return new Color(r, g, b, a);
    }

    private static int getChroma(long speed, long... delay) {
    ...
    private static float getEfficiency(final ItemStack itemStack, final Block block) {
        ...
        if (getStrVsBlock > 1.0f) {
            final int getEnchantmentLevel = net.minecraft.enchantment.EnchantmentHelper.getEnchantmentLevel(net.minecraft.enchantment.Enchantment.efficiency.effectId, itemStack);
    ...
    private static void drawRoundedGradientOutlinedRectangle(float x, float y, float x2, float y2, final float radius, final int n6, final int n7, final int n8) {
        x *= 2.0f;
        y *= 2.0f;
        x2 *= 2.0f;
        y2 *= 2.0f;
