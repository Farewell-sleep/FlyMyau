...
public class BlockHit extends Module {
        ...
        if (event.getType() == EventType.PRE) {
            ...
            if (this.mode.getValue() == 1) {
                ...
                if (target.isDead || !mc.theWorld.loadedEntityList.contains(target)
                        || mc.thePlayer.getDistanceToEntity(target) > this.range.getValue() * 2.0F) {
                    reset();
                    target = null;
                    return;
                }
            ...
            if (this.mode.getValue() == 2) {
                ...
                OpenMyau.lagManager.setDelay(delayPacketTick.getValue());
                if (blockTicks >= 1) {
                    KeyBindUtil.setKeyBindState(mc.gameSettings.keyBindUseItem.getKeyCode(), true);
                    blockTicks++;
                }
            ...
            } else OpenMyau.lagManager.setDelay(0);
        }
    }
    ...
    private void reset() {
        ...
        startBlocking = false;
        KeyBindUtil.updateKeyState(mc.gameSettings.keyBindUseItem.getKeyCode());
        holdTicks = sagTicks = blockTicks = 0;
        timer.reset();
    }
