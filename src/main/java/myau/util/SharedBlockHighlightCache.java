...
/**
 * SHARED BLOCK HIGHLIGHT CACHE — skidded from Raven B4 (keystrokesmod),
 * trimmed to the bed-feet branch only (BlockESP/BlockListSetting parts removed).
 * Shared per-world cache of bed feet positions, driven by chunk scans and
 * server block-change packets, with a dirty-listener mechanism.
 */
public final class SharedBlockHighlightCache {
    ...
    public void handleReceivePacket(Packet<?> packet) {
        if (!anyConsumerActive()) {
            return;
        }
        if (packet instanceof S23PacketBlockChange) {
            S23PacketBlockChange pkt = (S23PacketBlockChange) packet;
            onBlockChange(pkt.getBlockPosition(), pkt.getBlockState());
        } else if (packet instanceof S22PacketMultiBlockChange) {
            S22PacketMultiBlockChange pkt = (S22PacketMultiBlockChange) packet;
            ...
            S21PacketChunkData pkt = (S21PacketChunkData) packet;
            ...
            S26PacketMapChunkBulk pkt = (S26PacketMapChunkBulk) packet;
    ...
    private static final class BedFootHighlightMatcher {
        private boolean matchesBlock(IBlockState state) {
            return state != null && state.getBlock() instanceof BlockBed;
        }

        private boolean shouldIndexAt(BlockPos pos, IBlockState state) {
